...
class MetadataWatermark(WatermarkingMethod):
    ...
    def is_watermark_applicable(self, pdf: PdfSource, position: str | None = None) -> bool:
        """
        Checks if the PDF is well-formed enough for metadata manipulation.
        This method is applicable only to valid PDFs that can be parsed
        by pypdf and contain at least one page.
        """
        pdf_bytes = load_pdf_bytes(pdf)
        try:
            reader = pypdf.PdfReader(BytesIO(pdf_bytes))
            # The PDF must have pages to create a valid new PDF with metadata.
            if not reader.pages:
                return False
        except pypdf.errors.PdfReadError:
            # The file is not a valid PDF that pypdf can read.
            return False
        except Exception:
